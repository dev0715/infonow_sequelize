...
export class TokenCore {
	static async VerifyJWT(token: string): Promise<any> {
		return new Promise((resolve, reject) => {
			JsonWebToken.verify(token, AuthConfigs.private_key!, (error: any, payload: any) => {
				if (error) {
					return reject(error)
				}
				if (!_.has(payload, 'iat') || !_.has(payload, 'exp'))
					return reject(new UnAuthorizedError('You have been signed out!'))

				const tokenExpiry = _vals.isUnixTimestamp(payload.exp)
				if (isNaN(tokenExpiry)) return reject(new UnAuthorizedError('You have been signed out!'))
	...
	static DecryptAES(payload: string): string {
		const decrypted = CryptoJS.AES.decrypt(payload, AuthConfigs.aes_key).toString(CryptoJS.enc.Utf8)
	...
	static IssueJWT(payload: any, tokenLife?: number): Promise<string> {
		return new Promise((resolve, reject) => {
			try {
				...
				JsonWebToken.sign(tokenPayload, AuthConfigs.private_key!, (err: any, encoded: any) => {
