...
export class AuthorizeUtil {
	...
	private static async AuthorizeCore(req: Request): Promise<User> {
		...
		if (authHeader.length == 0) throw new UnAuthorizedError('No Authorization Token provided!')
	 ...
	 * @param next NextFunction
	 */
	static async AuthorizeTeacher(req: Request, res: Response, next: NextFunction) {
		try {
	 ...
	 * @param next NextFunction
	 */
	static async AuthorizeStudent(req: Request, res: Response, next: NextFunction) {
		try {
			const ERR_MSG = 'Cannot Authorize the user at the moment.'
	...
	static async AuthorizeAdmin(req: Request, res: Response, next: NextFunction) {
		try {
			...
			if (req.CurrentUser.roleId != 'admin')
				throw new UnAuthorizedError(`${ERR_MSG} code:${AuthorizeUtil.ERR_CODES1.USER_TYPE_NOT_ADMIN}`)
	 ...
	 * @param next NextFunction
	 */
	static async AuthorizeSuperAdmin(req: Request, res: Response, next: NextFunction) {
		try {
			const ERR_MSG = 'Cannot Authorize the user at the moment.'
			...
			if (req.CurrentUser.roleId != 'super-admin')
				throw new UnAuthorizedError(`${ERR_MSG} code:${AuthorizeUtil.ERR_CODES1.USER_TYPE_NOT_SUPER_ADMIN}`)
	...
	static AuthorizeUserSession(req: Request, userId?: any, specificKey?: string) {
		...
		if (!_.has(req.CurrentUser, sessionKey)) throw new BadRequestError(ERR_MSG)

		// If Authorization Key is userId, it must be a UUID
		if (
			sessionKey == 'userId' &&
			(_vals.isUuid(req.CurrentUser?.userId ?? '') === false || _vals.isUuid(userId ?? '') === false)
		)
			throw new BadRequestError(ERR_MSG)

		// If Authorization key is not matching the Session Key
		if ((req.CurrentUser as any)![sessionKey] !== userId) throw new BadRequestError(ERR_MSG)
	}
}
